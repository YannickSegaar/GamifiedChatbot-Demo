...
// YRS: AI Form Extension - VERSION 4

// Stateful Quiz Extension - Expert Recommended Pattern
export const AIConversationalQuizExtension4 = {
  name: 'AIConversationalQuiz',
  type: 'response',
  match: ({ trace }) => trace.type === 'ext_ai_conversational_quiz4',

  render: ({ trace, element }) => {
    console.log('StatefulQuizExtension received payload:', trace.payload);
    
    // --- 1. Get Raw Data from Voiceflow Payload ---
    const {
      currentQuestionIndex = 0,
      questionsJSON = '[]',
    ...
    // --- 5. Core Logic ---
    function renderHistory() { /* Renders messages */ }
    function renderQuestion() { /* Renders the current question and options */ }
    async function handleAnswerSelection(event) { /* Handles button clicks, calls n8n, and sends data back to Voiceflow */ }
    ...
    async function handleAnswerSelection(event) {
      const selectedAnswer = event.target.getAttribute('data-answer');
      const currentQuestion = questions[currentQuestionIndex];
      const updatedHistory = [...conversationHistory, { role: 'user', content: selectedAnswer }];
      ...
      try {
        const response = await fetch(webhookUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ question: currentQuestion.text, answer: selectedAnswer, history: updatedHistory })
        });
        if (!response.ok) throw new Error(`Webhook failed: ${response.statusText}`);
        
        const data = await response.json();
        const aiResponse = data.reply || "Thanks for your answer!";
        const finalHistory = [...updatedHistory, { role: 'ai', content: aiResponse }];

        window.voiceflow.chat.interact({
          type: 'complete',
          payload: {
            updatedHistoryJSON: JSON.stringify(finalHistory),
            nextQuestionIndex: currentQuestionIndex + 1,
            isCompleted: currentQuestionIndex + 1 >= questions.length
          }
        });
...
export const AIConversationalQuizExtension5 = {
  ...
  render: ({ trace, element }) => {
    ...
    // --- Data is now directly in trace.payload ---
    const {
      questions = [],
      currentQuestionIndex = 0,
      webhookUrl = '',
    ...
    // --- Core Logic (with updated payload for Voiceflow) ---
    function renderHistory() { /* ... */ }
    function renderQuestion() { /* ... */ }
    async function handleAnswerSelection(event) { /* ... */ }
    ...
    async function handleAnswerSelection(event) {
      ...
      try {
        ...
        // Send updated variables back to Voiceflow
        window.voiceflow.chat.interact({
          type: 'complete',
          payload: {
            updatedHistory: finalHistory, // Send back the full array
            nextQuestionIndex: currentQuestionIndex + 1,
            isCompleted: currentQuestionIndex + 1 >= questions.length
          }
        });
        ...
        window.voiceflow.chat.interact({
          type: 'complete',
          payload: {
            updatedHistory: updatedHistory, // Send back history without AI response
            nextQuestionIndex: currentQuestionIndex + 1,
            isCompleted: currentQuestionIndex + 1 >= questions.length,
            error: error.message
          }
    ...
    async function handleAnswerSelection(event) {
      ...
      try {
        ...
        window.voiceflow.chat.interact({
          type: 'complete',
          payload: {
            updatedHistory: JSON.stringify(finalHistory),
            nextQuestionIndex: currentQuestionIndex + 1,
            isCompleted: currentQuestionIndex + 1 >= questions.length
          }
        });
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 1

export const HawaiianSnorkelQuizExtension1 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz1' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz1',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
    ...
    const questions = [
      {
        ...
        options: [
          { id: 'q1a', text: 'Learning about the reef ecosystem', image: 'https://images.unsplash.com/photo-1551201927-347a35415b78?q=80&w=2070&auto=format&fit=crop', points: { m: 2, f: 1 }, funFact: 'Fantastic choice! Our tours are led by certified marine naturalists who can teach you how to identify fish and explain the lifecycle of the honu (Hawaiian Green Sea Turtle).' },
          { id: 'q1b', text: 'Finding a secluded, peaceful cove', image: 'https://images.unsplash.com/photo-1542257920-9a467053c2a8?q=80&w=2070&auto=format&fit=crop', points: { r: 2, m: 1 }, funFact: 'You\'ll love this! We specialize in visiting less-crowded "turtle cleaning stations" where you can observe turtles in a calm, natural setting away from the big tour boats.' },
          { id: 'q1c', text: 'Action-packed ocean adventure', image: 'https://images.unsplash.com/photo-1605333152748-52b364871964?q=80&w=2070&auto=format&fit=crop', points: { a: 2, f: 1 }, funFact: 'Awesome! Our "Ocean Explorer" package combines snorkeling with a thrilling zodiac boat ride along the coast to see sea caves and lava tubes up close.' },
          { id: 'q1d', text: 'Easy, safe fun for the whole family', image: 'https://images.unsplash.com/photo-1590523743849-ab43d35503b6?q=80&w=2069&auto=format&fit=crop', points: { f: 2, r: 1 }, funFact: 'Perfect! We provide high-quality life vests, flotation devices, and have in-water guides to ensure everyone from kids to grandparents has a safe and amazing time.' },
        ]
      },
      {
        id: 'q2-turtle',
    ...
    wrapper.innerHTML = `
      <style>
        ...
        .option {
          ...
          align-items: center;
          border: 2px solid ${borderColor};
          border-radius: 8px; /* --hte-radius-md */
          padding: 12px;
          cursor: pointer;
          transition: all 0.2s ease;
          background-color: ${accentColor};
        ...
        .option.selected {
          border-color: ${primaryColor};
          background-color: #F5F5F5; /* --hte-gray-lightest */
        }
        ...
        .option-image {
          ...
          height: 60px;
          object-fit: cover;
          border-radius: 4px; /* --hte-radius-sm */
          margin-right: 15px;
        }
      ...
      <div class="workflow-content">
        <div id="intro-step" class="workflow-step active">
          <img class="intro-image" src="https://images.unsplash.com/photo-1597787443599-3b64621c8b3f?q=80&w=2070&auto=format&fit=crop" alt="Sea turtle swimming in Hawaii">
        ...
        <div id="lead-capture-step" class="workflow-step">
            <h3 class="intro-title">Get Your Full Itinerary!</h3>
            <p class="intro-description">Enter your details below, and we'll email your snorkel persona and a custom guide for your recommended tour.</p>
            <form id="lead-form">
                <div class="form-group">
                    <label for="user-name">Name</label>
                    <input type="text" id="user-name" name="name" required>
    ...
    function renderFooterButtons() {
        ...
        switch(currentStep) {
            ...
            case 'results':
                 buttonsHTML = `
                    <a href="#" id="book-now-btn" target="_blank" class="btn btn-primary">Book This Tour</a>
                    <button id="email-results-btn" class="btn btn-secondary">Email My Results</button>
                `;
                break;
            case 'lead-capture':
                buttonsHTML = `
                    <button id="back-to-results-btn" class="btn btn-secondary">Back</button>
                    <button id="submit-lead-btn" class="btn btn-primary">Send My Itinerary</button>
                `;
                break;
            default:
                buttonsHTML = '';
    ...
    function handleCompletion(leadData = {}) {
      ...
      if (window.voiceflow?.chat) {
        ...
        console.log(`Quiz Complete! Persona: ${finalPersona.name}. Lead: ${leadData.name}, ${leadData.email}`);
        alert(`Quiz Complete! Your persona is: ${finalPersona.name}.`);
      }
    }
    ...
    function setupEventListeners() {
        ...
        if(submitLeadBtn) submitLeadBtn.onclick = (e) => {
            ...
            if (nameInput.value && emailInput.value && emailInput.checkValidity()) {
                handleCompletion({ name: nameInput.value, email: emailInput.value });
                const formContainer = el('#lead-capture-step');
                formContainer.innerHTML = `<h3 class="intro-title">Mahalo!</h3><p class="intro-description">Your personalized snorkel guide is on its way to your inbox.</p>`;
                el('#footer-buttons').innerHTML = `<button id="restart-btn" class="btn btn-primary">Start Over</button>`;
                ...
                // A simple validation message without using alert()
                ...
                errorMsg.textContent = 'Please enter a valid name and email address.';
            }
        };
    }
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 2

export const HawaiianSnorkelQuizExtension2 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz2' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz2',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 3

export const HawaiianSnorkelQuizExtension3 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz3' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz3',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
    ...
    wrapper.innerHTML = `
      <style>
        ...
        .intro-description, .question-description, .demo-explanation {
          font-family: 'Open Sans', sans-serif;
          font-size: 16px;
          color: #4A4A4A; /* --hte-gray-text */
          line-height: 1.6;
      ...
      <div class="workflow-header">
        <h2 id="header-title">Gamified Quiz Demo</h2>
      </div>
      ...
      <div class="workflow-content">
        ...
        <!-- RENAMED ORIGINAL INTRO STEP -->
        <div id="quiz-start-step" class="workflow-step">
          <img class="intro-image" src="https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/HawaiiTourExperts_logo.png" alt="Hawaii Tours Logo">
          <h3 class="intro-title">Discover Your Inner Honu</h3>
          <p class="intro-description">Answer 5 quick questions to reveal your unique Hawaiian snorkel style and get a personalized turtle tour recommendation!</p>
        </div>
    ...
    function renderFooterButtons() {
        ...
        switch(currentStep) {
            case 'intro':
                buttonsHTML = `<button id="start-btn" class="btn btn-primary">Take the Quiz!</button>`;
                break;
            case 'quiz-start':
                buttonsHTML = `<button id="start-quiz-btn" class="btn btn-primary">Start Quiz</button>`;
                break;
            case 'question':
                buttonsHTML = `
                    <button id="back-btn" class="btn btn-secondary">Back</button>
    ...
    function setupEventListeners() {
        ...
        if(startBtn) startBtn.onclick = () => showStep('quiz-start');
        if(startQuizBtn) startQuizBtn.onclick = () => showStep('question');
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 4

export const HawaiianSnorkelQuizExtension4 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz4' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz4',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
    ...
    wrapper.innerHTML = `
      <style>
        ...
        .form-group input { width: 100%; padding: 12px; border-radius: 8px; border: 1px solid ${borderColor}; font-size: 1rem; font-family: 'Open Sans', sans-serif; }
        .form-group input:focus { outline: none; border-color: ${primaryColor}; box-shadow: 0 0 0 2px rgba(139, 90, 150, 0.2); }
      ...
      <div class="workflow-content">
        ...
        <!-- NEW LEAD CAPTURE STEP FOR N8N -->
        <div id="contact-step" class="workflow-step">
            <h3 class="intro-title">Want More Info?</h3>
            <p class="intro-description">Our tour experts would be happy to contact you with more details about your recommended tour and other options. Just fill out your details below!</p>
            <form id="lead-form">
                <div class="form-group">
                    <label for="user-name">Name</label>
                    <input type="text" id="user-name" name="name" required>
        ...
        if(submitLeadBtn) submitLeadBtn.onclick = async (e) => {
            ...
            if (nameInput.value && emailInput.value && emailInput.checkValidity()) {
                ...
                if (success) {
                    ...
                    formContainer.innerHTML = `<h3 class="intro-title">Mahalo!</h3><p class="intro-description">Thank you! Our team will contact you shortly with more information.</p>`;
                    el('#footer-buttons').innerHTML = `<button id="restart-btn" class="btn btn-primary">Start Over</button>`;
                    setupEventListeners(); // Re-attach for the new restart button
                } else {
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 6

export const HawaiianSnorkelQuizExtension6 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz6' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz6',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
    ...
    wrapper.innerHTML = `
      <style>
        ...
        /* --- Lead Capture Form (IMPROVED STYLING) --- */
        .form-group { margin-bottom: 1.25rem; position: relative; }
        .form-group label { display: block; margin-bottom: 0.5rem; font-weight: 600; color: #4A4A4A; }
        .form-group .input-icon { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; color: #9E9E9E; transition: color 0.2s ease; }
        .form-group input { width: 100%; padding: 14px 14px 14px 48px; border-radius: 8px; border: 1px solid ${borderColor}; font-size: 1rem; font-family: 'Open Sans', sans-serif; transition: all 0.2s ease; }
        .form-group input::placeholder { color: #BDBDBD; }
        .form-group input:focus { outline: none; border-color: ${primaryColor}; box-shadow: 0 0 0 3px rgba(139, 90, 150, 0.2); }
        .form-group input:focus + .input-icon { color: ${primaryColor}; }
        .checkbox-group { display: flex; align-items: center; margin-top: 1.5rem; }
        .checkbox-group input { margin-right: 10px; width: 18px; height: 18px; accent-color: ${primaryColor}; }
        .checkbox-group label { margin-bottom: 0; font-size: 14px; }
        .checkbox-group a { color: ${primaryColor}; text-decoration: underline; }

        /* --- Thank You & Sending Screen (IMPROVED STYLING) --- */
        .centered-container { display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; height: 100%; padding: 20px; }
        .thank-you-container { background-image: linear-gradient(rgba(255,255,255,0.85), rgba(255,255,255,0.85)), url('https://images.unsplash.com/photo-1507525428034-b723a996f6ea?q=80&w=2070&auto=format&fit=crop'); background-size: cover; background-position: center; }
        .centered-container .intro-title { color: ${highlightColor}; }
        .centered-container .intro-description { color: #333; font-size: 18px; }
        .thank-you-icon { font-size: 48px; margin-bottom: 16px; color: ${primaryColor}; animation: wave 2s ease-in-out infinite; }
        ...
        <div id="contact-step" class="workflow-step">
            ...
            <form id="lead-form">
                ...
                <div class="checkbox-group">
                    ...
                    <label for="privacy-policy">I agree to be contacted and accept the <a href="#" target="_blank">Privacy Policy</a>.</label>
                </div>
            </form>
        </div>
        
        <!-- NEW SENDING/LOADING STEP -->
        <div id="sending-step" class="workflow-step">
            <div class="centered-container">
                <div class="spinner"></div>
                <h3 class="results-title" style="margin-top: 20px;">Sending Your Request...</h3>
            </div>
        </div>
    ...
    function renderFooterButtons() {
        ...
        switch(currentStep) {
            ...
            case 'contact':
                ...
                break;
            default:
                buttonsHTML = ''; // Hide buttons on sending/thank you screens
                break;
        }
        footer.innerHTML = buttonsHTML;
        setupEventListeners();
    ...
    async function sendToWebhook(leadData) {
        ...
        try {
            ...
            console.error('Error sending data to webhook:', error);
            // In a real scenario, you might not want to alert the user, but for the demo it's ok.
            // Returning true to not block the user flow in the demo even if webhook fails.
            return true; 
        }
    }
    ...
    function setupEventListeners() {
        ...
        if(submitLeadBtn) submitLeadBtn.onclick = async (e) => {
            ...
            const emailInput = el('#user-email');
            const phoneInput = el('#user-phone');
            const privacyCheckbox = el('#privacy-policy');
            ...
            showStep('sending'); // Show loading spinner
            ...
            if (success) {
                const formContainer = wrapper.querySelector('.workflow-content');
                formContainer.innerHTML = `
                ...
                alert('There was an error submitting your request. Please try again.');
                showStep('contact'); // Go back to the contact form
            }
        };
    }
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 7

export const HawaiianSnorkelQuizExtension7 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz7' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz7',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
    ...
    // --- Other Tours Data for Carousel ---
    const otherTours = [
        {
            title: 'Ka Moana Luau at Aloha Tower',
            description: 'Experience Polynesian traditions with interactive dances and an island-style buffet at this award-winning luau.',
            image: 'https://images.unsplash.com/photo-1580539845979-20173c459f27?q=80&w=2070&auto=format&fit=crop',
            url: '#'
        },
        {
            title: 'Diamond Head Luau',
            description: 'Enjoy a unique dining experience at the Waikiki Aquarium, complete with a spectacular show of Polynesian dance, song, and fire.',
            image: 'https://images.unsplash.com/photo-1602219542092-563a2f883766?q=80&w=2070&auto=format&fit=crop',
            url: '#'
        },
        {
            title: 'Sights & Bites Island Food Tour',
            description: 'Explore Oahu\'s best sights and get a true taste of local flavors on this fun sightseeing and food tour combo.',
    ...
    wrapper.innerHTML = `
      <style>
        ...
        .hawaii-quiz-wrapper * { box-sizing: border-box; }
        .workflow-header { background: ${primaryColor}; color: white; padding: 18px 20px; text-align: center; }
        .workflow-header h2 { margin: 0; font-family: 'Montserrat', sans-serif; font-size: 19px; font-weight: 600; letter-spacing: 0.5px; }
        .workflow-content { flex: 1; overflow-y: auto; padding: 30px 25px 90px 25px; position: relative; font-family: 'Open Sans', sans-serif; }
        .workflow-step { display: none; animation: fadeIn 0.4s ease-in-out; }
        .workflow-step.active { display: flex; flex-direction: column; }
        ...
        /* --- Calculating Step --- */
        .calculating-container, .centered-container { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; text-align: center; }
        .spinner { width: 60px; height: 60px; border: 5px solid rgba(33, 150, 243, 0.3); border-top-color: ${secondaryColor}; border-radius: 50%; animation: spin 1s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        ...
        /* --- NEW: Tour Carousel Styling --- */
        .thank-you-container { padding: 0 !important; }
        .carousel-title { padding: 0 20px; }
        .tour-carousel { position: relative; width: 100%; height: 400px; overflow: hidden; border-radius: 12px; margin-top: 20px; }
        .carousel-track { display: flex; height: 100%; transition: transform 0.5s ease-in-out; }
        .carousel-slide { min-width: 100%; height: 100%; position: relative; color: white; }
        .carousel-slide img { width: 100%; height: 100%; object-fit: cover; }
        .carousel-slide .overlay { position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(to top, rgba(0,0,0,0.8), transparent); padding: 40px 20px 20px; }
        .carousel-slide h4 { font-family: 'Montserrat', sans-serif; margin: 0 0 5px 0; font-size: 1.2rem; }
        .carousel-slide p { margin: 0 0 15px 0; font-size: 0.9rem; }
        .carousel-slide .btn-small { padding: 8px 16px; font-size: 0.8rem; background: ${accentColor}; color: ${primaryColor}; border-radius: 20px; text-decoration: none; }
        .carousel-arrow { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.7); border: none; border-radius: 50%; width: 40px; height: 40px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        ...
        .btn-primary { background-color: ${primaryColor}; color: white; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); }
        .btn-primary:hover:not(:disabled) { background-color: ${highlightColor}; transform: translateY(-2px); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
        .btn-primary:disabled { background-color: #BDBDBD; cursor: not-allowed; opacity: 0.8; }
        .btn-secondary { background-color: ${accentColor}; color: ${primaryColor}; border: 2px solid ${primaryColor}; }
        .btn-secondary:hover { background-color: ${primaryColor}; color: white; }
      </style>
      ...
      <div class="workflow-content">
        ...
        <div id="calculating-step" class="workflow-step">
          <div class="centered-container">
            <div class="spinner"></div>
            <h3 class="results-title" style="margin-top: 20px;">Finding Your Perfect Wave...</h3>
          </div>
        </div>
        ...
        <!-- NEW THANK YOU STEP WITH CAROUSEL -->
        <div id="thank-you-step" class="workflow-step">
             <div class="thank-you-container">
                <h3 class="intro-title carousel-title">Mahalo! We'll Be In Touch.</h3>
                <p class="intro-description carousel-title">While you wait, check out some of our other popular adventures!</p>
                        ...
                        ${otherTours.map(tour => `
                            <div class="carousel-slide">
                                ...
                                <div class="overlay">
                                    <h4>${tour.title}</h4>
                                    <p>${tour.description}</p>
                                    <a href="${tour.url}" target="_blank" class="btn-small">Learn More</a>
                                </div>
                            </div>
                        `).join('')}
                    </div>
    ...
    function showStep(stepId) {
      ...
      if (targetStep) {
        ...
        if (stepId === 'question') renderCurrentQuestion();
        if (stepId === 'results') renderResults();
        if (stepId === 'thank-you') initCarousel(); // Initialize carousel when this step is shown
      }
    }
    ...
    function renderFooterButtons() {
        ...
        switch(currentStep) {
            ...
            case 'results':
                 buttonsHTML = `
                    <button id="contact-btn" class="btn btn-primary">Talk To Us</button>
                    <button id="restart-btn" class="btn btn-secondary">Start Over</button>
                `;
                break;
            case 'contact':
    ...
    function initCarousel() {
        ...
        resetInterval(); // Start the autoplay
    }
...
// YRS: HAWAII TOUR EXPERTS TURTLE QUIZ - VERSION 8

export const HawaiianSnorkelQuizExtension8 = {
  name: 'HawaiianSnorkelQuiz',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_hawaiianSnorkelQuiz8' || trace.payload?.name === 'ext_hawaiianSnorkelQuiz8',
  render: ({ trace, element }) => {
    // --- Configuration from Voiceflow Payload ---
    const {
      workflowTitle = 'Find Your Hawaiian Snorkel Style',
    ...
    const otherTours = [
        {
            title: 'Ka Moana Luau at Aloha Tower',
            description: 'Experience Polynesian traditions with interactive dances and an island-style buffet at this award-winning luau.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_KaMoanaLuau.jpg',
            url: '#'
        },
        {
            title: 'Diamond Head Luau',
            description: 'Enjoy a unique dining experience at the Waikiki Aquarium, complete with a spectacular show of Polynesian dance, song, and fire.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_DiamondHead.jpg',
            url: '#'
        },
        {
            title: 'Pearl Harbor Arizona Memorial',
            description: 'Visit Pearl Harbor\'s historic museum and memorial to pay your respects to the fallen sailors and brave heroes of December 7, 1941.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_PearlHarbor.jpg',
            url: '#'
        },
        {
            title: 'Sights & Bites Island Food Tour',
            description: 'Explore Oahu\'s best sights and get a true taste of local flavors on this fun sightseeing and food tour combo.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_Sights&Bites.jpg',
            url: '#'
        },
        {
            title: 'Fine Dining & City Lights Tour',
            description: 'Treat yourself to fine dining at Roy\'s Restaurant, followed by an evening under the stars at a spectacular Honolulu viewpoint.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_FineDining.jpg',
            url: '#'
        },
        {
            title: 'Circle Island Snorkeling Tour',
            description: 'Join our lifeguard-certified guides for a safe and exciting one-day snorkeling trip around the island while learning local history.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_CircleIslandSnorkeling.jpeg',
            url: '#'
        },
        {
            title: 'North Shore Adventure',
            description: 'Escape Waikiki to explore the North Shore\'s Jurassic mountains and beautiful beaches while sampling local coffee and chocolate.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_NorthShore.jpg',
            url: '#'
        },
        {
            title: 'Private Guided Tours',
            description: 'Create your own custom, private tour for any group size by choosing your own unique destinations and activities.',
            image: 'https://yannicksegaar.github.io/GamifiedChatbot-Demo/HawaiiTourExperts_Quiz/Tour_PrivateGuidedTours.jpg',
            url: '#'
        }
    ];
    ...
    function renderFooterButtons() {
        ...
        switch(currentStep) {
            ...
            case 'contact':
                ...
                break;
            case 'thank-you':
                buttonsHTML = `<a href="https://fareharbor.com/embeds/book/alohahawaiitours/items/76226/calendar/2025/08/?sheet-uuid=bb32709d-5e68-4af4-93dc-ad298be9d4f1&asn=hteaffiliate&flow=9334&language=en-gb&full-items=yes&back=https://www.hawaiitourexperts.com/&g4=yes" target="_blank" class="btn btn-primary">Book Now!</a>`;
                break;
            default:
                buttonsHTML = '';
                break;
